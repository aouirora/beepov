...
ACCENT   = "#B5863C"   # muted amber — the "bee" colour
ACCENT_D = "#936B2C"

# Muted, grown-up category colours (used for the hexagon map pins + legend)
CATEGORY_COLORS = {
    "Food & Drink":        "#B07C3A",
    "Nightlife":           "#7E6597",
...
# --- Floating legend / count / call-to-action chips + district description.
#     My additions, themed via the design tokens above so they match the scheme. ---
st.markdown(
    f"""
    <style>
        /* Floating legend + result count (bottom-left over the map) */
        .bee-float-legend {{
        ...
        /* Floating contextual call-to-action (top-center over the map) */
        .bee-float-cta {{
        ...
        /* Short district description under the name in the Overview panel */
...
if (selected_district != "City View (No Pins)"
    ...
    # Safety net: if the shared DB connection briefly returns nothing during
    # overlapping reruns, treat it as an empty result instead of crashing.
    if df_all is None:
        df_all = pd.DataFrame()
...
# ==============================================================================
# 9. FLOATING LEGEND / RESULT COUNT / CONTEXTUAL GUIDANCE  (over the map)
# ==============================================================================
if df_pins is not None and not df_pins.empty:
    _legend_items = [
    ...
    _legend_rows = "".join(
        f'<span class="item"><span class="dot" style="background:{color}"></span>{escape(label)}</span>'
        for label, color in _legend_items
    )
    _count = len(df_pins)
    st.markdown(
        f"""
        <div class="bee-float-legend">
            <div class="count">📍 {_count} place{"s" if _count != 1 else ""} shown</div>
            <div class="row">{_legend_rows}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )

...
if selected_district != "City View (No Pins)":
    ...
    if parent_categories_selected:
        ...
        if st.sidebar.button("Shuffle Recommendations"):
            st.session_state["shuffle_seed"] = datetime.utcnow().isoformat()

        st.sidebar.markdown("---")
        st.sidebar.subheader("5. Bee Ratings")
        st.sidebar.caption("Use community Bees to surface places people enjoyed.")
        minimum_bee_rating = st.sidebar.slider(
            "Minimum average Bee rating:",
            min_value=0,
            max_value=5,
            value=0,
            format="%d Bees"
...
if selected_district != "City View (No Pins)" and (active_subcategories or show_public_transport) and os.path.exists('data/berlin_pois.parquet'):
    ...
    query = """
        SELECT name, district_name, master_category, subcategory, wkt_geometry 
    ...
    # Add stable randomization and limit to curate the view.
    query += " ORDER BY hash(coalesce(name, '') || ?) LIMIT ?"
    params.extend([st.session_state["shuffle_seed"], max_results])
...
if df_pins is not None and not df_pins.empty:
    ...
    df_pins["bee_average"] = rating_summaries.apply(lambda summary: summary[0])
    df_pins["bee_count"] = rating_summaries.apply(lambda summary: summary[1])
